...
import "../../../style/components/content/Post.scss";
...
class Post extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.activeSubmodule !== nextProps.activeSubmodule) {
      this.setState({
        moduleIndex: nextProps.activeModule,
        subModuleIndex: nextProps.activeSubmodule[2]
      });
    }
  }
  ...
  componentDidMount() {
    this.setState({
      moduleIndex: this.props.activeModule,
      subModuleIndex: this.props.activeSubmodule[2]
    });
  }
  ...
  isLastModule = () => {
    ...
    return (
      !this.state.modules[module_index] ||
      typeof this.state.modules[module_index] === "undefined"
    );
  };

  onClickPreviousModule = () => {
    let indexOfActiveSubmodule = this.props.moduleKeys.indexOf(
      this.props.activeSubmodule
    );
    this.props.setActiveSubmodule(
      this.props.moduleKeys[indexOfActiveSubmodule - 1]
    );
  ...
  renderPreviousButton = () => {
    ...
    return (
      <Button
        bsStyle="primary"
        className="btn-left"
        onClick={this.onClickPreviousModule}
        bsSize="large"
      >
        Wstecz
      </Button>
  ...
  renderNextButton = () => {
    if (!this.isLastSubmodule()) {
      return (
        <Button
          bsStyle="primary"
          onClick={this.onClickNextSubmoduleButton}
          bsSize="large"
        >
          Dalej
        </Button>
      ...
      if (this.isLastModule()) {
        return (
          <Button
            bsStyle="primary"
            onClick={this.onClickStartLastTest}
            bsSize="large"
          >
            Test końcowy
          </Button>
        ...
        return (
          <Button
            bsStyle="primary"
            onClick={this.onClickNextModuleButton}
            bsSize="large"
          >
            Następny moduł
          </Button>
